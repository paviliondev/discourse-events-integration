...
describe DiscourseEventsIntegration::Auth::Meetup do
  let(:state) { "1234" }
  let(:code) { "1234567" }
  let(:client_id) { "12345" }
  ...
  let(:provider) {
    Fabricate(:discourse_events_integration_provider,
  ...
  it "generates an authorization url" do
    auth = DiscourseEventsIntegration::Auth::Meetup.new(provider.id)
